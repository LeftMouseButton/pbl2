...
\author{
\IEEEauthorblockN{Duncan J. et al
}
...
\begin{abstract}
This study presents the design and analysis of a comprehensive \textbf{knowledge graph (KG)} that integrates information regarding cancer diseases, symptoms, treatments, and genes. 
...
Biomedical data are fragmented across thousands of web resources, each describing diseases, genes, and therapies in distinct textual formats.
...
HTML tags and boilerplate sections (e.g., “Start Here”) are removed, and text is normalized for large-language-model (LLM) processing. 
This ensures compatibility with token constraints while maintaining biomedical accuracy.
...
The graph is built using \textbf{NetworkX}, where each entity type corresponds to a node label and each relation forms a typed edge (e.g., \texttt{has\_symptom}, \texttt{associated\_gene}, \texttt{treated\_by}). 
Interactive visualization is provided using \textbf{PyVis}.

\subsection{Tools and Frameworks}
Python 3.12, NetworkX, PyVis, Pandas, and Leiden/Louvain community-detection algorithms were employed.
Adamic–Adar, Jaccard, and Preferential Attachment were used for link prediction, while ChatGPT-5 supported interpretive analysis.
...
\begin{figure}[H]
    ...
    \caption{Full cancer knowledge graph visualization (interactive PyVis export).}
    \label{fig:fullgraph}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth]{figs/cancer_kg_full.png}
    \caption{Community detection output showing modular clusters (Louvain method).}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth]{figs/cancer_kg_full.png}
    \caption{Centrality metrics (degree, betweenness, eigenvector) highlighting major hubs.}
...
\[
Breast~cancer \rightarrow BRCA1 \rightarrow DNA~repair \rightarrow Chemotherapy~response
\]
...
\begin{itemize}
    ...
    \item LLM extraction errors can introduce noise.
    \item Token constraints restrict large-scale ingestion beyond 300 diseases without summarization.
\end{itemize}
