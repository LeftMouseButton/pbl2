...
The cancer knowledge graph represents entities across multiple biomedical layers, including \textit{diseases, genes, treatments, symptoms, risk factors, biomarkers, and biological pathways}. Each entity type corresponds to a node label, while their interactions (e.g., \textit{associated\_gene}, \textit{treated\_by}, \textit{has\_symptom}) form typed edges. This schema allows both molecular and clinical aspects of oncological data to coexist within a unified graph, enabling cross-domain reasoning and multi-scale analysis.
